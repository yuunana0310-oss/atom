...
class WriterAgent:
    ...
    def _build_prompt(self, research_item, pattern, profile, feedback):
        ...
        # 【2026-04-12 新規】構造分析ガイドラインをフィードバックから読み込む
        ...
        effective_opening = structure_guidelines.get("most_effective_opening_style", "declaration_statement")
...
- 口癖・個性: {char_quirks}（口調は標準語ベース。方言は使わない。「〜と思うんよ」のような柔らかい言い回しは可だが、「〜やで」「〜やけん」「〜だけん」「〜してみ」などの讃岐弁は使用禁止）
...
- 政治・選挙
- スポーツ
- 【重要】特定の患者の「自主トレ指導」や「リハビリ手技」に関する偏った体験談（※この話題ばかりになるのを避けること。働き方、AI活用、チーム連携、日々の気づきなど、医療職・専門職にとって『多様でためになる普遍的な話題』を心がける）
...
### 📏 文字数・改行のルール
- **推奨文字数**: {char_count_guide}（短めが勝ち。全体平均より32%短い投稿がいいね獲得率2倍以上）
- **推奨改行数**: {line_breaks_guide}（スマートフォン視認性最優先。1改行あたり15-20字の目安）
- **禁止**: リスト形式の過剰使用（伸びる投稿は30%以下に抑える）
...
- ✅ パターン: 「〇〇をやってわかったこと」
- ✅ キャラ感: 個人視点の前景化（「理学療法士の自分が」「管理職22年の経験から」など）
- ❌ NG: 「〜についての解説」「一般的な〇〇」など無個性な前置き

### ⭐ 投稿パターンの優先順位
1. **最優先: {top_pattern}** （個人体験＋共感型。平均いいね4.0個がトップ）
...
2. **【重要】{char_count_guide}（改行を含む）。短く簡潔にまとめることが最重要。分析より短めが勝ちの傾向**
3. 使用禁止ワードは一切使わない
...
11. 【返信を狙う投稿を1割含める】生成するバッチのうち約1件は「返信特化型」にすること：
    ...
    - POST2（body）は自分の体験・本音を2〜4行で語る。答えを押しつけず「僕はこう」「自分はこう思うけど」で終わる
    - 末尾は「〜って人いる？」「みんなどうしてるんだろう」など標準語で自然に余韻を残す
...
### POST1（hook）
- 【重要／禁止事項】POST1（フック）の文末を絶対に「、」で終わらせないこと！「〇〇なんだよね、」のように中途半端な読点で終わると不自然で気持ち悪さを与えます。必ず「。」（句点）、「？」（疑問符）、または「体言止め」でしっかり文を言い切って（完結させて）ください。— 0.5秒でスクロールを止めることだけが目的
...
余計な説明は不要です。JSONのみ出力してください。"""
    ...
    def _build_prompt_fixed(self, profile, feedback):
        """
        固定部分のプロンプト（キャラクター・ルール・フォーマット定義）
        Prompt Cachingの対象となる約3,000トークン。
        """
        character = profile.get("character", {})
        char_identity = character.get("role_identity", "")
        char_quirks = character.get("quirks", "")
...
## 扱わないテーマ（絶対NG）
以下のテーマは内容・例示・言及を含め一切触れない：
- 投資・株・FX / 仮想通貨・暗号資産 / 転職・就職系 / 政治・選挙 / スポーツ
...
## 投稿作成ルール（共通）
1. Threads向けの自然な日本語。「教科書的な正解」は捨て、毒気と偏愛を込めること。
2. {char_count_guide}（改行を含む）。短く簡潔にまとめることが最重要。
3. 使用禁止ワードは一切使わない
4. 【絶対ルール】文章の長さとスレッド分割：
   - short_complete / comment_hook / demand_check / expose_type / myth_bust → **6行以内必須**。thread_replies は空リスト。
   - 上記以外 → **8行以内**に収め、続きを thread_replies に分ける（最大3件）。
5. 【最重要】1行目は「0.5秒で止まる」インパクト。数字+結果・逆説・本音・問いかけ・短い断言を使う。
6. 押しつけがましくなく、自然な流れで読者の行動を促す
7. 【数字・固有名詞を必ず入れる】抽象表現より具体的な数字・ツール名・年数が信頼を生む
8. 【1投稿1メッセージ】複数の気づきを詰め込まない
9. ハッシュタグは使わない
10. noteへの誘導・URLの記載は一切しない
11. 【返信を狙う投稿を1割含める】問いかけで読者の体験・意見を引き出す
12. 【感情表現のルール】ネガティブな感情語を避ける（「ショック」「怒った」など）
13. 【コンテンツ配分：3層構造で作る】
    - A枠（5割）：誰にでも刺さる普遍テーマ。医療職以外の読者獲得入口
    - B枠（3割）：専門職全般・AI興味層に刺さる
    - C枠（2割）：医療職に深く刺さる
14. 【オリジナル視点を必ず入れる】「PT歴22年の現役管理職」にしか言えない視点を入れること

## 投稿フォーマット（2投稿セット）
すべての投稿を **POST1（フック）＋ POST2（本文）** の2投稿セットで生成すること。

### POST1（hook）
- 【重要／禁止事項】POST1（フック）の文末に「、」を使わないこと。必ず「。」（句点）、「？」（疑問符）、または「体言止め」で自然に文章を言い切って（完結させて）ください。中途半端な「、」終わりは不自然です。
- **1〜3行のみ**。絶対に超えない。1行が理想。
- 15文字以内の短い断言・問いかけ・本音が最強。
...
## 【重要】高精度な単一生成モード
内部の思考プロセスで「問いかけ型」「数字型」「逆説型」「本音告白型」「宣言型」など様々な切り口を検討し、その中で最も読者の反応（conversation_potential）が高くなると判定した**最高の1件のみ**を出力してください。複数の出力は不要です。"""

        return fixed_prompt

    def _build_prompt_variable(self, research_item, pattern, feedback):
        """
        可変部分のプロンプト（テーマ・パターン・参考情報）
        毎回変わるため、キャッシュ対象外。約500トークン。
        """
        theme = research_item.get("theme", "転職")
        title = research_item.get("title", "")
        description = research_item.get("description", "")
...
- 説明: {pattern['description']}
- 例文: {pattern.get('example', '')}
{"- 【このパターンは短文必須】改行を含めて6行以内で書くこと。絶対に超えないこと。" if pattern["id"] in ("short_complete","comment_hook","demand_check","expose_type","myth_bust") else "- 【このパターンは最大8行まで】それ以上はNG。"}
...
## 出力形式
以下のJSONフォーマットで、検討した結果最も優れている**1件のみ**を出力してください。
...
採点基準：
- conversation_potential: 読者がコメントしたくなるか。8未満は再生成対象。
- original_viewpoint: 「PT歴22年の管理職」にしか言えない内容か。8未満は再生成対象。
- single_message: メッセージが1つに絞られているか。7未満は再生成対象。
- specificity: 数字・固有名詞・ツール名が入っているか。7未満は再生成対象。

余計な説明は不要です。JSONオブジェクトのみ出力してください。"""
    ...
    def _parse_claude_response_array(self, response_text):
        """
        Parse Claude's JSON array response (5パターン同時生成用).
        返り値: [投稿1, 投稿2, ..., 投稿5] のリスト or None
        """
        json_str = None
        ...
        if cb_match:
            ...
            if start != -1 and end != -1 and end > start:
                ...
                if start != -1 and end != -1:
                    ...
                    logger.warning("_parse_claude_response_array: No JSON found in response.")
                    return None
        ...
        try:
            ...
            if isinstance(result, list):
                return result
            logger.warning("_parse_claude_response_array: Result is neither dict nor list.")
            return None
        except json.JSONDecodeError as e:
            snippet = response_text[:200].replace("\n", " ")
            logger.error(f"_parse_claude_response_array: JSON decode error: {e} | snippet: {snippet!r}")
            return None
    ...
    def _select_best_candidate(self, candidates, pattern, ng_words_list, recent_texts):
        """
        5パターン中から最高スコアの1つを選別する。

        Args:
            candidates: list[dict] - Claude が生成した5パターン
            pattern: dict - パターン定義
            ng_words_list: list[str] - NGワード
            recent_texts: list[str] - 過去100投稿のテキスト

        Returns:
            dict - 最高スコアのpost entry or None（全候補不合格）
        """
        valid_candidates = []

        for i, raw_candidate in enumerate(candidates):
            # Step A: 正規化・検証
            normalized = self._normalize_and_validate_post(raw_candidate, pattern, ng_words_list)
            if not normalized:
                logger.info(f"  [Candidate {i+1}] normalization failed")
                continue
            ...
            if not passed:
                logger.info(f"  [Candidate {i+1}] score check failed: {reason}")
                continue
            ...
            if max_sim >= self.similarity_threshold:
                logger.info(f"  [Candidate {i+1}] similarity too high ({max_sim:.3f})")
                continue

            valid_candidates.append((avg_score, normalized))
            logger.info(f"  [Candidate {i+1}] PASS (avg_score={avg_score:.2f})")

        if not valid_candidates:
            logger.info(f"_select_best_candidate: all {len(candidates)} candidates rejected")
            return None

        # スコア最高の候補を選択
        valid_candidates.sort(key=lambda x: x[0], reverse=True)
        best_score, best_candidate = valid_candidates[0]
        logger.info(f"_select_best_candidate: selected best candidate (score={best_score:.2f} from {len(valid_candidates)} valid)")
    ...
    def _generate_with_claude(self, fixed_part=None, variable_part=None, prompt=None):
        """
        Call Claude API with Prompt Caching.

        2つの実行モード：
        1. マルチパターン（固定・可変分離）: fixed_part + variable_part を渡す
        2. 従来型（後方互換）: prompt を渡す
        """
        ...
        try:
            ...
            system_text = """あなたはThreads投稿の専門家です。PT歴22年の現役管理職になりきり、『中間管理職の葛藤・リーダーシップ・現場と経営の板挟み・組織マネジメント』を主軸に一人称体験談を生成してください。

【方針転換：患者指導ではなく「管理職目線」を大増量】
これまでのデータから「患者への指導術・教育」等より「管理職としての本音・苦悩・若手との関わり」のほうが圧倒的に読者の反応が良いため、テーマを大きく管理職目線へシフトします。
- メイン（70%）: 管理職の葛藤・板挟み・部下育成・キャリアの悩み・職場の人間関係
- サブ（20%）: 現場あるある（※患者より「制度」「環境」「自身の経験」を語る）
- 情報系（10%）: AI（Claude等）を活用して理不尽な業務をどうハックしたか

【ターゲット読者の3層】
●メイン70%：PT/OT/STの中堅〜管理職層および一般の社会人（30-50歳）
 悩み：上と下の板挟み、部下が育たない、組織の無駄なルール、将来への不安
 刺さる：管理職の本音、ベテランの失敗談、理不尽にどう対処したかの知恵

●サブ20%：若手・中堅スタッフ（25-35歳）
 悩み：この職場でいいのか、給与の不満、上司のホンネが知りたい
 刺さる：評価者目線のアドバイス、上司が裏で考えていること

●隠し10%：一般社会人・他業種（40-60歳）
 悩み：仕事と家庭の両立、部下との関わり方
 刺さる：人間関係の普遍的な悩み、リーダーとしての孤独体験

【毒気（独自視点）の5つの核心 - 必ず投稿に1つ以上】

●毒気1：22年の現場でしか見えない『患者さんのリアル』
 例：「『頑張ります』と言う患者さんの9割は伝わってない。本音の確認法」
 内容：教科書や若手には書けない、現場の空気感・人間の機微。

●毒気2：失敗談→気づきの転換（自己開示）
 例：「PT1年目、担当患者さんに『他の人がいい』と言われた日。あの夜のこと」
 内容：恥ずかしい失敗を包み隠さず話す。そこから学んだことを共有。

●毒気3：医療業界のタブー・構造的問題に切り込む
 例：「医者の指示のまま、他人事のままでは患者は続けない」
 内容：業界内で『言いづらいこと』を現役ベテランの立場で言語化する。

●毒気4：管理職×現場の板挟み経験
 例：「上からはコスト削減、下からは人員不足。その中でリハ職としてできること」
 内容：他の現役PTには書けない、管理職の視点からの葛藤。

●毒気5：実装可能な3ステップ・具体的な質問法
 例：「『なぜ』を患者さんに言語化させる質問：『あなたの身体で何が起きてると思う？』」
 内容：明日から現場で使える、再現性のある具体策。


- 本文は『共感→原因→3ステップ（or 具体策）→まとめ』の構造で、末尾「。」で自然終了
- 口調は標準語ベース。讃岐弁などの方言は使わない（「〜と思うんよ」程度の柔らかい言い回しは可）
- 「ぶっちゃけ」「正直」「これマジで」「控えめに言って」など心情語を1-2個
- 「前は〜、今は〜」という対比構造で変化を明示する



【投稿テーマの選択指針】
1. 管理職の葛藤・中間管理職の板挟み・組織マネジメント（A枠）← ★超最優先（最も鉄板）
2. 個人のキャリア・副業や働き方の本音（B枠）
3. AI（Claude）を使った理不尽な業務の突破（C枠）
4. 現場あるある・22年PTとしての深い気づき（D枠）
5. noteティーザー・告知（E枠）← 週1-2本のみ
6. コメント誘発型（F枠）← 週1-2本

【🚨 テーマ強制変換ルール（患者指導ネタの排除）】
参考キーワードが『患者向けの話』『ホームエクササイズ』等であっても、そのまま「患者への接し方」として書くことは禁止です！
必ず『それをスタッフにどう指導するか』『管理職としてどうその問題に直面しているか』『組織の課題としてどう受け止めているか』という【管理職・指導者の葛藤目線】に強制変換して語ってください。

【避けるべき内容】
❌ Claude Code・AI技術の説明だけ
❌ 「AIで効率化した」の抽象論
❌ 医療×AI副業の売り込み色が強いもの
❌ プロフ誘導・note誘導の連投
✅ 22年現場のリアル + 患者さんとの関係 + 実装可能な知恵
✅ リハ職なら全員が『あるある』と頷く悩み + 独自の解決策
✅ 失敗談からの学び（自己開示）"""

            # マルチパターンモード（fixed_part + variable_part）
            if fixed_part and variable_part:
                message = client.messages.create(
                    model=self.model,
                    max_tokens=1024,  # 出力1件のみのため削減
                    system=[
                        {
                            "type": "text",
                            "text": system_text,
                            ...
                            "content": [
                                {
                                    "type": "text",
                                    "text": fixed_part,
                                    "cache_control": {"type": "ephemeral"}  # 固定部分をキャッシュ（90%削減効果）
                                },
                                {
                                    "type": "text",
            ...
            # 従来モード（後方互換）
                ...
                logger.error("_generate_with_claude: Either (fixed_part + variable_part) or prompt must be provided")
                return None
...
- ハッシュタグは入れない
- JSON形式で返す
- 【最重要】body の最終行が「、」で終わっている場合、絶対にそのまま保持すること。「。」に変換禁止。これはコメント欄への誘導テクニックのため変更厳禁。
            ...
            while retry_count <= self.max_retries and not post_accepted:
                ...
                if using_claude:
                    ...
                    if not response_text:
                        ...
                        continue
                    # 既存の_parse_claude_response_arrayを利用して1件のリストとして受け取る
                    ...
                    if not candidates:
                        ...
                        continue
                    # 検証を通す（1件のみ）
                    logger.info(f"WriterAgent: Validating the generated candidate...")

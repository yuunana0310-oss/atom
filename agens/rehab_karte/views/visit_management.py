...
class VisitManagementView(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        ctk.CTkLabel(top, text="訪問管理", font=ctk.CTkFont(size=18, weight="bold")).pack(side="left")
        ctk.CTkButton(top, text="＋ 新規予定", width=110, command=self._new_schedule).pack(side="right")
        ...
        ctk.CTkEntry(search_frame, textvariable=self._search_var, width=200,
                     placeholder_text="患者名・種別・ステータス").pack(side="left", padx=6)
        ...
        ctk.CTkLabel(filter_frame, text="期間：").pack(side="left")
    ...
    def _refresh(self):
        ...
        if not schedules:
            ctk.CTkLabel(self._table, text="該当する予定がありません", text_color="gray").grid(
                row=1, column=0, columnspan=len(COLUMNS), pady=20)
            return

        for r, s in enumerate(schedules, start=1):
            values = [
                s["scheduled_date"],
                s["scheduled_time"] or "",
                s["patient_name"],
            ...
            btn_frame.grid(row=r, column=5, padx=4)

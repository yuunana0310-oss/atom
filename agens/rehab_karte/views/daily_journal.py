...
class DailyJournalView(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        ctk.CTkLabel(left, text="保存済み日誌", font=ctk.CTkFont(weight="bold")).grid(
            row=0, column=0, sticky="ew", padx=8, pady=(8, 4)
        )
        self._list_frame = ctk.CTkScrollableFrame(left)
        self._list_frame.grid(row=1, column=0, sticky="nsew", padx=4, pady=4)
        ...
        # 日付バー
        date_bar = ctk.CTkFrame(right, fg_color="transparent")
        date_bar.grid(row=0, column=0, sticky="ew", padx=12, pady=(12, 4))
        date_bar.columnconfigure(1, weight=1)

        ctk.CTkLabel(date_bar, text="日付：").grid(row=0, column=0, sticky="w")
        self._date_var = ctk.StringVar(value=self._current_date)
        ctk.CTkEntry(date_bar, textvariable=self._date_var, width=120).grid(
            row=0, column=1, sticky="w", padx=6
        )
        ctk.CTkButton(date_bar, text="読み込む", width=90,
                      command=self._on_load_click).grid(row=0, column=2, padx=4)
        ctk.CTkButton(date_bar, text="↻ 下書き生成", width=110,
                      command=self._generate_draft).grid(row=0, column=3, padx=4)

        # マイクボタンの追加
        self._mic_btn = ctk.CTkButton(date_bar, text="🎙️ マイク録音", width=110, fg_color="#F44336", hover_color="#D32F2F", command=self._toggle_mic)
        self._mic_btn.grid(row=0, column=4, padx=4)
        
        self._is_recording = False

        # ステータスラベル
        self._status_label = ctk.CTkLabel(right, text="", text_color="gray",
                                          font=ctk.CTkFont(size=12))
        self._status_label.grid(row=1, column=0, sticky="w", padx=14)

        # ---- 当日記録一覧 ----
        rec_header = ctk.CTkFrame(right, fg_color="transparent")
        rec_header.grid(row=2, column=0, sticky="ew", padx=12, pady=(4, 0))
        ctk.CTkLabel(rec_header, text="当日の実施記録",
                     font=ctk.CTkFont(weight="bold")).pack(side="left")
        ctk.CTkButton(rec_header, text="＋ 記録追加", width=100,
                      command=self._add_record).pack(side="right")

        self._records_frame = ctk.CTkScrollableFrame(right, height=160)
        self._records_frame.grid(row=3, column=0, sticky="ew", padx=12, pady=(2, 4))
        self._records_frame.columnconfigure(1, weight=1)

        # テキストエリア
        self._textbox = ctk.CTkTextbox(right, font=ctk.CTkFont(size=13))
        self._textbox.grid(row=4, column=0, sticky="nsew", padx=12, pady=6)
        right.rowconfigure(4, weight=1)

        # ボタン行
        btn_bar = ctk.CTkFrame(right, fg_color="transparent")
        btn_bar.grid(row=5, column=0, sticky="e", padx=12, pady=(0, 12))
        ctk.CTkButton(btn_bar, text="保存", width=90,
                      command=self._save).pack(side="left", padx=4)
        self._confirm_btn = ctk.CTkButton(
            btn_bar, text="確定", width=90,
